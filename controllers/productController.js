...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const uniqueName = Date.now() + file.fieldname + "." + extension;
    cb(null, uniqueName);
  },
});
...
const productController = {
  async store(req, res, next) {
    ...
    handleMultipartData(req, res, async (err) => {
      ...
      try {
        const alreadyExist = await Product.exists({ name: req.body.name });
        if (alreadyExist) {
            fs.unlink(`${appRoot}/${filePath}`, (err) => {
              if (err) {
                return next(CustomErrorHandler.serverError(err.message));
          ...
          return next(
            CustomErrorHandler.alreadyExist("Product already Exist.")
          );
        }
      } catch (err) {
        return next(err);
